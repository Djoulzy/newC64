...
func (V *VIC) Init(ram *memory.MEM, io *memory.MEM, chargen *memory.MEM, video graphic.Driver) {
	...
	V.io = io
	V.chargen = chargen
	V.color = (io.GetView(colorStart, 1024)).(*memory.MEM)
	V.screen = (ram.GetView(screenStart, 1024)).(*memory.MEM)

	V.io.Write(REG_EC-IOStart, 0xFE)  // Border Color : Lightblue
	V.io.Write(REG_B0C-IOStart, 0xF6) // Background Color : Blue
	V.io.Write(REG_CTRL1-IOStart, 0b10011011)
	V.io.Write(REG_RASTER-IOStart, 0b00000000)
	V.io.Write(REG_CTRL2-IOStart, 0b00001000)
	V.io.Write(REG_IRQ-IOStart, 0b00001111)
	V.io.Write(REG_SETIRQ-IOStart, 0b00000000)
...
func (V *VIC) saveRasterPos(val int) {
	V.io.Write(REG_RASTER-IOStart, byte(val))
	raster := V.io.Read(REG_CTRL1 - IOStart)
	if (byte(uint16(val) >> 8)) == 0x1 {
		V.io.Write(REG_CTRL1-IOStart, raster|RST8)
	} else {
		V.io.Write(REG_CTRL1-IOStart, raster & ^RST8)
	}
